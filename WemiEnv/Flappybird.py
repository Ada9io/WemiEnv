...
class FlappybirdEnv():
    def __init__(self):
        ...
        self.action_meanings = "action =0; =1 Fly; =2 Reset"
        # 状态空间，当状态空间有多个向量时，会把多个向量看作多个样例，返回对应的action列表（返回numpy类型的对象）
        self.observation_space = spaces.Box(low=0, high=600, shape=(3, ), dtype=np.float32)
        # self.observation_space = spaces.Box(low=-1000, high=1000, dtype=np.int32)
        # 当前观测状态
        self.cur_obs = None
        # 服务端创建
